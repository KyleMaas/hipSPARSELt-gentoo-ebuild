...
# Modifications copyright 2025 Kyle Maas
...
S="${WORKDIR}/rocm-libraries-rocm-${PV}/projects/hipsparselt/"
...
PATCHES=(
	"${FILESDIR}"/hipSPARSELt-7.1.0-fix-lib.patch
)
...
src_unpack() {
	...
	local HIPSPARSELT="rocm-libraries-rocm-${PV}/projects/hipsparselt"
	local ORIGAMI="rocm-libraries-rocm-${PV}/shared/origami"
	tar -xzf "${DISTDIR}/${A}" "${HIPBLASLT}" "${HIPSPARSELT}" "${ORIGAMI}" -C "${WORKDIR}" || die
}
